...
use crate::{dynamic_json::{parse_to_dynamic_value, DynamicValue}, parse::{Section, SpannedDiagnostic}, DclFile};
...
pub fn parse_resource_section<'a>(dcl: &mut DclFile, section: &Section<'a>) -> Result<(), SpannedDiagnostic> {
    ...
    let input = parse_to_dynamic_value(&body_concat).map_err(|e| {
        let mut diag = SpannedDiagnostic::new(format!("failed to parse resource '{}' body: {}", resource_name, e), section.start_line, 999);
