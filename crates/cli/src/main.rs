...
async fn main() {
    ...
    let dcl_file = std::fs::read_to_string(&dcl_file_path)
        ...
        .unwrap();
    if let Err(diagnostics) = main_or_err(dcl_file).await {
...
async fn main_or_err(
    ...
    let (state, state_file_path) = cli_engine::load_state(&dcl).map_err(|e| vec![e])?;
    let logger = log::logger();
    let state = cli_engine::perform_update(logger, dcl, state).await
